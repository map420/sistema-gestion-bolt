...
export default function DetalleModal({ trabajador, registros, pagos, periodo, onCerrar }: Props) {
  ...
  const etiquetaPeriodo = `${periodo.desde} ${t('payments.to')} ${periodo.hasta}`
      ...
      <div className="bg-[#111] border border-white/10 rounded-2xl w-full max-w-lg p-6 flex flex-col gap-4 max-h-[80vh]">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-[#555]">{t('detail.period')} {etiquetaPeriodo}</p>
          </div>
        ...
        <div className="overflow-y-auto flex flex-col gap-1.5">
          {registros.length === 0 ? (
            <p className="text-sm text-[#555] text-center py-8">{t('detail.noRecords')}</p>
          ) : registros.map(r => (
            <div key={r.id} className="flex items-center gap-3 bg-[#0d0d0d] border border-white/5 rounded-lg px-3 py-2.5">
              <span className="text-xs text-[#555] w-20 shrink-0 capitalize">{fmtFecha(r.fecha).split(',')[0]}</span>
        ...
        {registros.length > 0 && (
          <div className="border-t border-white/5 pt-3 flex justify-between items-center gap-4">
            <div className="flex flex-col items-center gap-0.5">
              <span className="text-[10px] text-[#555] uppercase tracking-wide">{t('detail.earned')}</span>
              <span className="text-sm font-bold text-[#f0f0f0]">{fmt(devengado)}</span>
            </div>
            <div className="flex flex-col items-center gap-0.5">
              <span className="text-[10px] text-[#555] uppercase tracking-wide">{t('detail.paid')}</span>
              <span className="text-sm font-bold text-[#34d399]">{fmt(pagado)}</span>
            </div>
            <div className="flex flex-col items-center gap-0.5">
              <span className="text-[10px] text-[#555] uppercase tracking-wide">{t('detail.pending')}</span>
